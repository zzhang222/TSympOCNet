...
def plot_plane():
    ...
    for i in range(num_d):
        ...
        if i % 2:
            q, p = predict_qp(act, transform[i], get_latent[i], net_params, latent_params, t, t_terminal, qp[i])
        else:
            q, p = predict_qp(act, transform[i], get_latent[i], net_params, latent_params, t, t_terminal)
        d = ds[i//2]
        q, v = q[:, :d], q[:, d:]
        u = p[:, d:]
        num_drone = d // phy_dim
        ...
        for j in range(num_t):
            ...
            if i == 0:
                plt.title(f't = {ts[j] / 99 * 10:.1f}', fontsize = 18)
            if j == 0:
            ...
            for k in range(num_drone):
                patch = plt.Circle((q[ts[j], 2*k], q[ts[j], 2*k+1]), dr, fill = False, color = 'black')
                ax.add_patch(patch)
                arrow_v = plt.arrow(q[ts[j], 2*k], q[ts[j], 2*k+1], v[ts[j], 2*k], v[ts[j], 2*k+1], width = 0.01, fill = False, color = 'red')
                arrow_u = plt.arrow(q[ts[j], 2*k], q[ts[j], 2*k+1], u[ts[j], 2*k], u[ts[j], 2*k+1], width = 0.01, fill = False, color = 'blue')
                plt.plot(q[:ts[j]+1, 2*k], q[:ts[j]+1, 2*k+1], c = 'gray', alpha = 0.1)
            if i == 0 and j == 0:
...
if __name__ == '__main__':
    ...
    get_latent = [pinn, get_latent_pre, pinn, get_latent_pre]
    transform = [identity, t_sympnet, identity, t_sympnet]
    t_terminal = 10
    num_t = 100
    halfroomsz = 0.5
    Q_bd = []
    for numwall, numdperedge in [(1, 1), (2, 9)]:
    ...
    Cv = 0.5
    ds = [8, 128]
    ylabels = ['PINN', 'TSympOCNet', 'PINN', 'TSympOCNet']
    qp = []
    for i, d in enumerate(ds):
        for net_type in ['PINN', 'TSympNet']:  
            path = f'free_{net_type}_5_{d}_{Cv}_0.0_0.0'
